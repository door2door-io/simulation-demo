...
export default class Vehicle {
  ...
  addRouteToVehicle(coords) {
    ...
    const arc = []
    for (let i = 0; i < lineDistance * 100; i++) {
      const segment = turf.along(this.route, lineDistance * i / (lineDistance * 100), 'kilometers')
      arc.push(segment.geometry.coordinates)
    }
    this.route.geometry.coordinates = arc
    this.layers.addRoute(routeId, this.route, this.color)
  }
